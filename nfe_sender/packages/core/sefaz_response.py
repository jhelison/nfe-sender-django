...
class SefazCommunication:
    def __init__(self, cert: CertificateA1, hom: bool = True) -> None:
        self.cert = cert
        self.hom = hom
        self.cUF = 21
    ...
    def status_servico(self) -> dict:
        ...
        root = etree.Element("consStatServ", nsmap=nsmap, versao="4.00")
    ...
    def consulta_protocolo(self, chNFe: str) -> dict:
        ...
        nsmap = {None: "http://www.portalfiscal.inf.br/nfe"}
        root = etree.Element("consSitNFe", nsmap=nsmap, versao="4.00")

        root.append(self._element_with_text("tpAmb", 2 if self.hom else 1))
        root.append(self._element_with_text("xServ", "CONSULTAR"))
        root.append(self._element_with_text("chNFe", chNFe))

        res = self._post(URL, SERVICE, root)

        return res

    def _post(
        self, url: str, service: "str", etree_data: etree.Element
    ) -> etree.Element:
        with CertificateAsFile(self.cert) as (key, cert):
            ...
            client = False
            return self._children_to_dict(res)
    ...
    @staticmethod
    def _children_to_dict(root: etree.Element):
        child_dict = {}
